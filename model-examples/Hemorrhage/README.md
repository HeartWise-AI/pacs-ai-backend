...
```sh
docker build -t heartwisehub/hemorrhage:1.0.0 .
docker push heartwisehub/hemorrhage:1.0.0
```

To run the docker image locally for testing, run the following command:

```sh
docker run -p 8000:8000 --gpus all --shm-size 2g heartwisehub/hemorrhage:1.0.0
```
...
```sh
docker run -it --network pacs-net --gpus all --shm-size 2g --entrypoint /bin/bash heartwisehub/hemorrhage:1.0.0
```
