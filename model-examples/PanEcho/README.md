...
```
docker build -t cacoool/pacs-ai-pan-echo:1.0.0 .
docker push cacoool/pacs-ai-pan-echo:1.0.0
```

To run the docker image locally for testing, run the following command:
```
docker run -p 8000:8000 cacoool/pacs-ai-pan-echo:1.0.0
```

If you want to add GPU support, run the following command:
```
docker run -p 8000:8000 --gpus all cacoool/pacs-ai-pan-echo:1.0.0
```
...
```
docker run -it --network pacs-net --gpus all --entrypoint /bin/bash cacoool/pacs-ai-pan-echo:1.0.0
```