...
class CustomPredictionService(BasePredictionService):
    ...
    def stop_model(self):
        ...
        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
            try:
                if proc.info['name'] == 'tensorflow_model_server' or (proc.info['cmdline'] and 'tensorflow_model_server' in proc.info['cmdline'][0]):
                    proc.terminate()
    ...
    def _detect_devices(self, pixel_data):
        ...
        confident_indices = [i for i, score in enumerate(detection_scores) if score >= confidence_threshold]
    ...
    def _get_quality_predictions(self, instances):
        ...
        quality_result = requests.post(
            self.SERVER_QUALITY_URL,
            json={'instances': instances}
        ).json()
        
        image_quality = []
        for one_hot in quality_result['predictions']:
