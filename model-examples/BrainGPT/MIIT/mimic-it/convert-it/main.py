...
if __name__ == "__main__":
    ...
    print(f" type du dataset {type(dataset)}")# Debugging print
    print(f" contenu: {dataset}")# Debugging print
    print(f"Dataset short name: {dataset_short_name}") # Debugging print
    print(f"Dataset keys: {list(dataset.keys())[:5]}")  # Debugging print
    ...
    with open(f"output/{dataset_short_name}.json", "w") as f:
        ...
        for image_key, base64_data in get_json_data_generator(dataset, dataset_short_name, args.num_threads):
            if base64_data is None:
                continue  # skip this image if it's truncated
