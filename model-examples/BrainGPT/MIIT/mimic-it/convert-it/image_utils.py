...
def process_image(image: bytes, target_size=(224, 224)) -> bytes:
    ...
    try:
        ...
        if "image file is truncated" in str(e):
            print(f"Warning: Encountered a truncated image. Image data: {image[:100]}...")  # print the first 100 bytes for brevity
...
def get_json_data_generator(images: dict[str, bytes], dataset_name: str, num_threads: int) -> Generator[Tuple[str, str], None, None]:
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        def process_image_wrapper(args):
            ...
            print(f"Processing image: {new_key}", flush=True) # print for debugging
            result = get_b64_data(process_image(img))

            process_bar.update()
            return new_key, result
        print(f"il ya bien {len(images)}") # print for debugging
        for result in executor.map(process_image_wrapper, images.items()):
            if result[1] is not None:  # Check if base64_data is not None
                yield result
