...
class OtterConfig(PretrainedConfig):
    ...
    def __init__(self, vision_config=None, text_config=None, cross_attn_every_n_layers: int = 4, use_media_placement_augmentation: bool = True, **kwargs):
        ...
        if "architectures" in text_config.keys() and text_config["architectures"] != None:
            ...
            if not text_config or "model_type" not in text_config:
                import json
                import os

    # Lecture manuelle du vrai config.json
                config_path = kwargs.get("config_path", "./checkpoints/OTTER_CLIP_BRAINGPT_hf/config.json")
            ...
            if "model_type" not in text_config:
                raise ValueError(f"'model_type' not found in text_config: {text_config}")
