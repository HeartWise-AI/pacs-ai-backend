...
class BasePredictionService:
    ...
    def _convertFromBase64(self, inference):
        """
        Converts base64 encoded inferences to numpy arrays
        """
        # Moking the conversion with fake data
        import random
        video1 = np.random.randint(0, 256, (random.randint(25, 75), 512, 512), dtype=np.uint8)
    
        # Add the color channel dimension (C=1 for grayscale)
        video1 = np.expand_dims(video1, axis=1)  # Shape becomes (frames, 1, height, width)
        return video1
    ...
    def unload_model(cls):
        ...
        # Explicitly call garbage collector and empty CUDA cache (if CUDA is available)
        if torch.cuda.is_available():
            torch.cuda.empty_cache()  # Free up the memory used by the cache
