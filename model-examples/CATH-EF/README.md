...
```
docker build -t cacoool/cath-ef:1.6 .
docker push cacoool/cath-ef:1.6
```

To run the docker image locally for testing, run the following command:
```
docker run -p 8000:8000 cacoool/cath-ef:1.6
```

If you want to add GPU support, run the following command:
```
docker run -p 8000:8000 --gpus all cacoool/cath-ef:1.6
```
...
```
docker run -it --network pacs-net --gpus all --entrypoint /bin/bash cacoool/cath-ef:1.6
```
