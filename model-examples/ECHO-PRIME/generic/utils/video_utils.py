...
def load_pixel_array(dcm: dicom.Dataset) -> np.ndarray:
    ...
    native PixelData buffer is full YBR_FULL/RGB sized (1/3 larger). pydicom
    then refuses to decode. Retag before .pixel_array for that case; leave
    encapsulated (compressed) payloads to pylibjpeg.
    """
...
def apply_zoom(img_batch, zoom=0.1):
    ...
    return img_batch[:, pad_y:-pad_y, pad_x:-pad_x, :]
...
def mask_outside_ultrasound(original_pixels: np.array) -> np.array:
    ...
    try:
        ...
        # Dilate
        kernel = np.ones((3, 3), np.uint8)
        frame_overlap = cv2.dilate(np.uint8(frame_overlap), kernel, iterations=10).astype(
            np.uint8
        )
        ...
        contours, hierarchy = cv2.findContours(
            frame_overlap, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )
        ...
        frame_overlap = np.where(frame_overlap > 0, 1, 0).astype(
            np.uint8
        )  # make all non-0 values 1
...
def read_video(
    ...
    if res is not None:
        ...
        if region is None:
            ...
            out = np.zeros(
                (n_frames, region[1] - region[0], region[3] - region[2]), dtype=np.uint8
            )
