...
```
docker build -t cacoool/echo-prime:1.0 .
docker push cacoool/echo-prime:1.0
```

To run the docker image locally for testing, run the following command:
```
docker run -p 8000:8000 cacoool/echo-prime:1.0
```

If you want to add GPU support, run the following command:
```
docker run -p 8000:8000 --gpus all cacoool/echo-prime:1.0
```
...
```
docker run -it --network pacs-net --gpus all --entrypoint /bin/bash cacoool/echo-prime:1.0
```
