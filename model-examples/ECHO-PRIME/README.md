...
```
docker build -t cacoool/echo-prime:1.1 .
docker push cacoool/echo-prime:1.1
```

To run the docker image locally for testing, run the following command:
```
docker run -p 8000:8000 cacoool/echo-prime:1.1
```

If you want to add GPU support, run the following command:
```
docker run -p 8000:8000 --gpus all cacoool/echo-prime:1.1
```
