...
class EchoPrimeInference:
    ...
    def __init__(self, mil_weights_path: str, candidate_studies_path: str, candidate_embeddings_path: str, candidate_reports_path: str, candidate_labels_path: str, section_to_phenotypes_path: str, json_data_path: str, all_phrases_path: str):
        ...
        # Process phrases
        t_list = {k: [self.all_phrases[k][j] for j in self.all_phrases[k]] 
                for k in self.all_phrases}
        self.phrases_per_section_list = {k: functools.reduce(lambda a,b: a+b, v) 
                                      for (k,v) in t_list.items()}
        self.regex_per_section = {k: self._make_it_regex(v)
                               for (k,v) in self.phrases_per_section_list.items()}
    ...
    def get_views(self, stack_of_videos: torch.Tensor, view_classifier: torch.nn.Module,
        ...
        stack_of_view_encodings = torch.stack(
            [torch.nn.functional.one_hot(out_views, 11)]
    ...
    def embed_videos(self, stack_of_videos: torch.Tensor,
                    echo_encoder: torch.nn.Module) -> torch.Tensor:
    ...
    def encode_study(self, stack_of_videos: torch.Tensor,
    ...
    def _visualize_views(self, frames: torch.Tensor, view_list: List[str]) -> None:
        ...
        for i in range(len(view_list)):
            ...
            cv2.putText(
                display_image, view_list[i].replace("_", " "),
                (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 220, 255), 2
            )
    ...
    def process_dicoms(self, input_dir: Dict[str, Any]) -> torch.Tensor:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all tasks
            future_to_path = {
                executor.submit(self.process_single_dicom, path): path 
    ...
    def process_series_instance_metadata(self, seriesInstanceMetadata: Dict[str, Any]) -> torch.Tensor:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all tasks
            future_to_path = {
                executor.submit(self.process_single_series_instance_metadata, instances): instances
