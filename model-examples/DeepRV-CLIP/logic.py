...
class CustomPredictionService(BasePredictionService):
    DEFAULT_DATASET_MEAN = [133.98593139648438, 133.98593139648438, 133.98593139648438]
    DEFAULT_DATASET_STD = [27.26662254333496, 27.26662254333496, 27.26662254333496]
    ...
    def load_model(self, config: Config):
        ...
        print(f"DeepRV-CLIP loaded. CUDA: {torch.cuda.is_available()}")
    ...
    def _postprocess(self, outputs: dict[str, torch.Tensor]) -> dict[str, float]:
        ...
        for key, value in outputs.items():
            ...
            if task == "binary_classification":
                ...
                result[key] = float(v.item())
        return result

    def _format_predictions(self, preds: dict[str, float]) -> dict[str, object]:
        cm = CustomPredictionService._class_mapping
        prob = float(preds.get("Outcome", 0.0))
        threshold = cm["Outcome"]["threshold"]
        return {
            "abnormalRV": {
                "probability": round(prob, 3),
                "threshold": threshold,
                "diagnosis": "abnormal" if prob >= threshold else "normal",
            }
        }

    def _diagnosis_text(self, preds: dict[str, float]) -> str:
        prob = float(preds.get("Outcome", 0.0))
        threshold = CustomPredictionService._class_mapping["Outcome"]["threshold"]
        label = "Abnormal RV systolic function" if prob >= threshold else "Normal RV systolic function"
        return f"DeepRV-CLIP: {label} (P={prob:.2f})"
    ...
    def _recommendations(self, preds: dict[str, float]) -> dict[str, object]:
        ...
        if prob >= threshold:
            return {
                "en": (
                    f"<strong>Possible RV systolic dysfunction (P={prob:.2f}).</strong> "
                    "Consider transthoracic echocardiogram with dedicated RV assessment "
                    "(TAPSE, RV S', FAC) to confirm and quantify dysfunction."
                ),
                "fr": (
                    f"<strong>Dysfonction systolique ventriculaire droite probable (P={prob:.2f}).</strong> "
                    "Envisager une échocardiographie transthoracique avec évaluation VD dédiée "
                    "(TAPSE, S' du VD, FAC) pour confirmer et quantifier la dysfonction."
                ),
                "presentable": True,
            }
        return {
            "en": (
                f"<strong>Normal RV systolic function predicted (P={prob:.2f}).</strong> "
                "No additional RV-focused imaging indicated based on this prediction alone."
            ),
            "fr": (
                f"<strong>Fonction systolique du VD prédite normale (P={prob:.2f}).</strong> "
                "Aucune imagerie VD supplémentaire indiquée sur la seule base de cette prédiction."
            ),
            "presentable": True,
        }
    ...
    def _filter_dicoms_with_metadata(
        ...
        """DeepRV operates on both L and R coronary diagnostic frames."""
        allowed = {"Left Coronary", "Right Coronary"}
        filtered = []
        for dicom in dicoms:
            ...
            if not meta:
                continue
            if meta.get("main_structure") not in allowed:
                continue
    ...
    def _render_html(self, preds: dict[str, float], recs: dict[str, object]) -> str:
        ...
        badge_color = "#e74c3c" if is_abnormal else "#27ae60"
        badge_label = "Abnormal RV" if is_abnormal else "Normal RV"
        en = recs.get("en", "")
        pct = round(prob * 100, 1)

        html = f"""<!DOCTYPE html>
<html><head><meta charset=\"utf-8\"><title>DeepRV-CLIP Report</title>
<style>
...
<body><div class=\"card\">
 <h1>DeepRV-CLIP RV Systolic Function</h1>
 <div class=\"subtitle\">DeepCORO-CLIP linear-probe estimate from L+R coronary angiograms</div>
 <div class=\"metric\">
   <div><div class=\"val\">{pct}%</div><div class=\"lbl\">P(abnormal RV)</div></div>
   <div style=\"flex:1;text-align:right;\"><span class=\"badge\">{badge_label}</span><div style=\"margin-top:8px;color:#6c757d;\">Threshold = {threshold}</div></div>
 </div>
