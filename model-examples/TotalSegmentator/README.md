...
```sh
docker build -t cacoool/pacs-ai-segmentation:0.4.0 .
docker push cacoool/pacs-ai-segmentation:0.4.0
```

To run the docker image locally for testing, run the following command:

```sh
docker run -p 8000:8000 --gpus all --shm-size 2g cacoool/pacs-ai-segmentation:0.4.0
```
...
```sh
docker run -it --network pacs-net --gpus all --shm-size 2g --entrypoint /bin/bash cacoool/pacs-ai-segmentation:0.4.0
```
