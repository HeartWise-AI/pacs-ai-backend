...
```sh
docker run -p 8000:8000 --gpus all --shm-size 2g cacoool/pacs-ai-segmentation:1.0.0
```
...
```sh
docker run -it --network pacs-net --gpus all --shm-size 2g --entrypoint /bin/bash cacoool/pacs-ai-segmentation:1.0.0
```
