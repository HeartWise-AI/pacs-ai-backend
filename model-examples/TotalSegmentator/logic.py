...
class CustomPredictionService(BasePredictionService):
    def load_model(self, config: Config):
        ...
        with open(class_mapping_filepath) as f:
    ...
    async def _handle_ohif_output(self, request: PredictRequest):
        ...
        dicoms = [base64.b64decode(instance) for instance in series.values()]

        return self._handle_ohif(dicoms)
    ...
    def _handle_ohif(self, dicoms: dict[int, dict[int, dict]]) -> dict:
        ...
        try:
            ...
            pred = CustomPredictionService.models["totalsegmentator"](
                scan["NII"], device="gpu", fastest=True, task="total"
            )
            ...
            if (
                ...
                and instances[0].ImagePositionPatient[2] > instances[1].ImagePositionPatient[2]
            ):
