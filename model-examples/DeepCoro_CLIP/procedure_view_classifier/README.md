...
```
docker build --secret id=hf_token,src=./hf_token.txt -t heartwisehub/pacs-ai-procedure_view-classifier:1.0 .
docker push heartwisehub/pacs-ai-procedure_view-classifier:1.0
```

To run the docker image locally for testing, run the following command:
```
docker run -p 8000:8000 heartwisehub/pacs-ai-procedure_view-classifier:1.0
```

If you want to add GPU support, run the following command:
```
docker run -p 8000:8000 --gpus all heartwisehub/pacs-ai-procedure_view-classifier:1.0
```
...
```
docker run -it --network pacs-net --gpus all --entrypoint /bin/bash heartwisehub/pacs-ai-procedure_view-classifier:1.0
```
