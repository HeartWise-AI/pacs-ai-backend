...
class CustomPredictionService(BasePredictionService):
    def load_model(self, config: Config):
        ...
        model_state_dict = torch.load(CustomPredictionService.model_path, map_location=torch.device('cpu'), weights_only=True)['linear_probing']
        ...
        CustomPredictionService.models['video_mil_wrapper'].to('cuda' if torch.cuda.is_available() else 'cpu')
        CustomPredictionService.is_initialized = True
            
        print('Model loaded')

    def _get_diagnosis(self, predictions: Dict[str, Union[float, torch.Tensor]]) -> Dict[str, str]:
    ...
    def _get_recommendations(self, predictions: Dict[str, Union[float, torch.Tensor]], language: str) -> str:
        ...
        artery_names = {
            ...
            "lad_stenosis_binary": {"en": "LAD (Left Anterior Descending)", "fr": "IVA (Interventriculaire Antérieure)"},
            ...
            "om1_stenosis_binary": {"en": "OM1 (Obtuse Marginal 1)", "fr": "OM1 (Marginale Obtuse 1)"},
            "om2_stenosis_binary": {"en": "OM2 (Obtuse Marginal 2)", "fr": "OM2 (Marginale Obtuse 2)"},
            ...
            "pda_stenosis_binary": {"en": "PDA (Posterior Descending)", "fr": "IVP (Interventriculaire Postérieure)"},
            "posterolateral_stenosis_binary": {"en": "Posterolateral Branch", "fr": "Branche Postérolatérale"}
        }
        ...
        if blocked_arteries:
            ...
            if language == "fr":
                arteries_text = ", ".join([f"{name} ({perc:.1f}%)" for name, perc in blocked_arteries])
                return (f"Sténose coronarienne détectée dans: {arteries_text}. "
                ...
                arteries_text = ", ".join([f"{name} ({perc:.1f}%)" for name, perc in blocked_arteries])
                return (f"Coronary stenosis detected in: {arteries_text}. "
            ...
            if language == "fr":
                return ("Aucune sténose coronarienne significative détectée. "
                       "Continuer la surveillance clinique de routine selon les protocoles établis.")
            else:  # English
                return ("No significant coronary stenosis detected. "
    ...
    def _run_inference(self, dicoms: List[pydicom.Dataset])->float:
        try:
            ...
            video_batch = video_batch.unsqueeze(0).to('cuda' if torch.cuda.is_available() else 'cpu')
            with torch.no_grad():
                output: torch.Tensor = CustomPredictionService.models['video_mil_wrapper'](video_batch)
