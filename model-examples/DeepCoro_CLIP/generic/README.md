...
```
docker build --build-arg HF_API_KEY=API_KEY_HERE -t heartwisehub/deepcoro-clip/pacs-ai-generic:1.0 .
docker push heartwisehub/deepcoro-clip/pacs-ai-generic:1.0 .
```

To run the docker image locally for testing, run the following command:
```
docker run -p 8000:8000 heartwisehub/deepcoro-clip/pacs-ai-generic:1.0 .
```

If you want to add GPU support, run the following command:
```
docker run -p 8000:8000 --gpus all heartwisehub/deepcoro-clip/pacs-ai-generic:1.0 .
```
