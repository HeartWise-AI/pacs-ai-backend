...
def test_all_nine_heads_are_postprocessed_and_grouped_in_report(service):
    outputs = {
        head: torch.tensor([[2.0 if index % 2 else -2.0]])
        for index, head in enumerate(service._class_mapping)
    }
    outputs["mace_urgent_revascularization_binary"] = torch.tensor([[2.0]])
    preds = service._postprocess(outputs)
    assert set(preds) == set(service._class_mapping)
    assert all(0.0 < probability < 1.0 for probability in preds.values())
    assert preds["mace_binary"] == pytest.approx(torch.sigmoid(torch.tensor(-2.0)).item())

    formatted = service._format_predictions(preds)
    assert formatted["timeHorizon"] == "1 year"
    assert len(formatted["primary"]) == 4
    assert len(formatted["exploratory"]) == 5
    assert formatted["primary"]["urgentRevascularization"]["aboveResearchThreshold"] is True
...
def test_empty_video_does_not_abort_later_valid_video_and_padding_mask(service, monkeypatch):
    ...
    class RecordingModel(torch.nn.Module):
        def forward(self, batch, video_mask):
            ...
            return {"mace_binary": torch.tensor([[1.0]])}
    ...
    assert outputs["mace_binary"].item() == 1.0
    assert seen == ["1.1", "1.2"]
...
def test_actual_mil_logits_are_exactly_invariant_to_padded_content(service):
    ...
    mil = MultiInstanceLinearProbing(
        ...
        pooling_mode="cls_token", attention_hidden=8, dropout=0,
