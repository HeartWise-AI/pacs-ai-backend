...
def create_dicom_payload(dicom_paths: Union[str, List[str]], output_mode: str = "JSON", send_metadata_only: bool = False, group_series: bool = False):
    """
...
def main():
    ...
    parser.add_argument(
        '--extract-only',
        action='store_true',
        help='Only extract the payload without sending it'
    )
    
    parser.add_argument(
        '--save-payload',
        type=str,
        help='Save extracted payload to the specified JSON file'
    )
