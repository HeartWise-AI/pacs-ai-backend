...
def create_dicom_payload(
    ...
    """
    Read DICOM file(s), process the data, and create a payload for API request.
    ...
    for idx, dicom_path in tqdm(
        enumerate(dicom_paths, start=1), total=len(dicom_paths), desc="Processing DICOM files"
    ):
        try:
            # Read DICOM file
            ...
            instance_metadata = {
                "00280008": {"Value": [getattr(ds, "NumberOfFrames", 1)], "vr": "IS"},
                "00280002": {"Value": [getattr(ds, "SamplesPerPixel", 1)], "vr": "US"},
                "00280010": {"Value": [getattr(ds, "Rows", 0)], "vr": "US"},
                "00280011": {"Value": [getattr(ds, "Columns", 0)], "vr": "US"},
                "00280100": {"Value": [getattr(ds, "BitsAllocated", 8)], "vr": "US"},
                "00280101": {"Value": [getattr(ds, "BitsStored", 8)], "vr": "US"},
                "00101010": {"Value": [getattr(ds, "PatientAge", "29")], "vr": "AS"},
                "7FE00010": {"InlineBinary": pixel_base64, "vr": "OB"},
            }
...
def send_dicom_data(
    ...
    Returns:
        Server response JSON or None if request failed
    """
    # Create the payload
...
def main():
    ...
    parser.add_argument(
        "--extract-only", action="store_true", help="Only extract the payload without sending it"
    )

    parser.add_argument(
        "--save-payload", type=str, help="Save extracted payload to the specified JSON file"
    )
    ...
    if args.extract_only or args.save_payload:
        # Only extract the payload without sending
        payload = create_dicom_payload(
        ...
        if args.extract_only:
            print("Payload extracted successfully (not sent)")
            return payload
...
# Expose create_dicom_payload for external scripts to import and use
