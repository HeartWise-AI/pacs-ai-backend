...
class DynamicContainerTool(BaseTool):
    ...
    def __init__(self, name: str, description: str, args_schema: Type[BaseModel]=DicomPayloadInput):
    ...
    def _run(
        ...
        if self.name not in TOOL_CONTAINER_MAPPING:
            return self._create_error_response(f"No container information found for tool {self.name}")
    ...
    def _is_invalid_payload(self, payload: Any) -> bool:
        ...
        if isinstance(payload, str) and payload.startswith("{'__arg"):
            return True
...
def create_dynamic_tool(tool_info: Dict[str, Any], container_ip: str, port: int = 80) -> Optional[BaseTool]:
    """
...
def create_tool_for_container(container: Dict[str, Any]) -> Optional[BaseTool]:
    """
    ...
    if tool_info:
        logger.info(f"Retrieved tool info from {container_name}: {tool_info.get('name', 'unknown')}")
