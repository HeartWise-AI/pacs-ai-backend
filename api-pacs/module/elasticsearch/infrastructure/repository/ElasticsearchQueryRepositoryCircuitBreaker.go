...
func (repository *ElasticsearchQueryRepositoryCircuitBreaker) SearchLoginLogs(ctx context.Context, searchParam types.SearchParameter) (*search.Response, error) {
	...
	errors := hystrix.Go("search_login_logs", func() error {
		login, err := repository.ElasticsearchQueryRepositoryInterface.SearchLoginLogs(ctx, searchParam)
		if err != nil {
			return err
		}
...
func (repository *ElasticsearchQueryRepositoryCircuitBreaker) SearchAdminMemberLogs(ctx context.Context, searchParam types.SearchParameter) (*search.Response, error) {
	output := make(chan *search.Response, 1)
	hystrix.ConfigureCommand("search_admin_member_logs", config.Settings())
	errors := hystrix.Go("search_admin_member_logs", func() error {
		adminMember, err := repository.ElasticsearchQueryRepositoryInterface.SearchLoginLogs(ctx, searchParam)
		if err != nil {
			return err
		}
